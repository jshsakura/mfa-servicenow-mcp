...
> [!WARNING]
> **개인용으로 만든 도구입니다 — 사용에 따른 모든 책임은 본인에게 있습니다.** 이 프로젝트는 제작자 본인의 워크플로우를 위해 만들어졌습니다. 리스크는 최대한 최소화하고 있으나(읽기 전용 기본값, 쓰기 가드, dry-run 프리뷰, 모든 쓰기에 `confirm='approve'` 게이트), **실제 운영 ServiceNow 인스턴스**를 대상으로 동작합니다. 인스턴스에서 발생하는 결과에 대한 책임은 전적으로 사용자에게 있습니다. **어떠한 보증도 없이 "있는 그대로" 제공**됩니다(Apache-2.0, [LICENSE](LICENSE) 참조). 도구를 승인하기 전에 무엇을 하는지 반드시 확인하세요.
