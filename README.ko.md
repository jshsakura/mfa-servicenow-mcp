...
Claude Code, Cursor, Codex, OpenCode, Windsurf, VS Code Copilot, Gemini CLI, Zed 등에 아래 내용을 붙여넣으세요:
...
- core, standard, service desk, 포탈 개발자, 플랫폼 개발자용 도구 패키지 — `full`은 고급 사용자 전용 ([경고 참고](docs/TOOL_PACKAGES.ko.md))
...
전체 레퍼런스(모든 패키지, 상속 구조, 설정 방법): [도구 패키지 고급 가이드](docs/TOOL_PACKAGES.ko.md).
...
`playwright`와 `mfa-servicenow-mcp` 둘 다 고정하면 결정적으로 동작합니다. 한 번 `uvx --with "playwright==1.58.0" playwright install chromium`을 받으면, 핀을 직접 올리기 전까지는 추가 다운로드가 없습니다.
