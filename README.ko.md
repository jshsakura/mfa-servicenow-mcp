...
python -m playwright install chromium     # MFA/SSO 로그인용 Chromium
```

Python 3.10 이상이 필요합니다. 없으면 [python.org 인스톨러](https://www.python.org/downloads/)로 설치하세요 — Windows 보안 정책(Smart App Control)에서도 서명된 인스톨러라 그대로 통과합니다.

Chromium을 **미리** 받아두는 게 중요합니다. 첫 도구 호출 때 받으면 ~150 MB 다운로드가 MCP 호스트의 handshake 데드라인을 넘겨 `connection closed`로 실패할 수 있습니다.

> **가이드 설치.** 플래그 없이 `python -m servicenow_mcp setup`을 실행하면 번호 메뉴로 안내합니다(클라이언트·인증 방식을 번호나 이름으로 선택 — 자유 입력으로 헤맬 일 없음). 영어/한국어 지원(로케일 자동 감지, `SERVICENOW_MCP_LANG=ko|en`으로 강제).

> **mac/Linux에서 `externally-managed-environment` 에러가 나면** — Homebrew·배포판 파이썬은 [PEP 668](https://peps.python.org/pep-0668/)로 전역 pip 설치를 막습니다. python.org 인스톨러를 쓰거나, `uvx`로 가세요:
>
> ```bash
...
> ```
>
> uvx로 설치했다면 아래 config에서 `command`/`args`만 바꾸면 됩니다 — [uvx로 실행하기](#uvx로-실행하기-maclinux) 참고. `env` 블록은 동일합니다.
...
```json
{
  ...
  "mcp": {
    "servicenow": {
      "type": "local",
      "command": ["python", "-m", "servicenow_mcp"],
      "enabled": true,
      "environment": {
        "SERVICENOW_INSTANCE_URL": "https://your-instance.service-now.com",
        "SERVICENOW_AUTH_TYPE": "browser"
...
PEP 668로 pip이 막히는 mac/Linux에서는 `uvx`가 편합니다. `env` 블록은 위와 **완전히 동일**하고, `command`/`args`만 바꾸면 됩니다:
...
Codex는 `command = "uvx"`, OpenCode는 `"command": ["uvx", "--with", "playwright", "--from", "mfa-servicenow-mcp", "servicenow-mcp"]` 형태로 같은 값을 넣으면 됩니다.

> Windows에서는 uvx를 권하지 않습니다. uvx는 실행할 때마다 서명 없는 임시 실행 파일을 풀어 쓰는데, Smart App Control이 이를 차단합니다. Windows는 pip 경로를 쓰세요.
...
대부분은 위 [설치](#설치)(pip)면 충분합니다. 사내망은 두 갈래입니다:
...
python -m playwright install chromium     # Playwright가 올라갔으면 새 Chromium 빌드가 필요합니다
```
...
> **uvx 사용자.** uvx는 마지막으로 받은 버전을 캐시해서 계속 재사용합니다 — 새 릴리스가 자동 반영되지 않으니 `--refresh`로 직접 갱신하세요:
>
> ```bash
> uvx --refresh --with playwright --from mfa-servicenow-mcp servicenow-mcp --version
> uvx --with playwright playwright install chromium
