...
Windows Smart App Control이 켜져 있으면 uvx가 동작하지 않습니다. uvx는 실행할 때마다 서명 없는 임시 실행 파일을 풀어 쓰는데 SAC가 그걸 차단하기 때문입니다. Windows 업데이트 이후 갑자기 막혔다면 대개 이겁니다. 이럴 때 pip으로 가세요:
...
#### 멀티 인스턴스 (dev / test / prod)

dev / test / prod를 함께 쓴다면 **서버를 여러 개 띄우지 마세요.** `env`만 바꾸면 한 연결에서 전부 다룰 수 있습니다:
...
`SERVICENOW_INSTANCE_URL` 자리에 alias 목록이 들어가는 것뿐이고, `command`/`args`는 그대로입니다. 이렇게 하면:

- **운영 보호가 기본** — `allow_writes`를 안 준 alias는 읽기 전용입니다. 위 예시에서 `prod`는 쓰기가 아예 안 됩니다.
- **재시작 없이 다른 인스턴스 조회** — `sn_query(instance="prod", ...)` 처럼 읽기 도구에 `instance`를 넘기면 됩니다.
- **인스턴스 간 비교** — `compare_instances`로 dev와 prod의 같은 컴포넌트를 직접 대조합니다.
- **로그인 한 번** — 브라우저 세션을 alias끼리 공유합니다.

전체 규칙(쓰기 라우팅·게이트·`${ENV}` 참조)은 [여러 인스턴스 — 두 가지 방식](#프로필-vs-멀티-프로세스)에 있습니다. 클라이언트 화면에서 연결을 눈으로 갈라야 하는 경우만 거기 **B. 멀티 프로세스**를 보세요.
...
대부분은 위 [설치](#설치)면 충분합니다. 사내망에서 짚어둘 경우가 둘 있습니다.

가장 흔한 경우는 **PyPI는 되는데 HTTPS가 TLS 검사되는** 환경(Zscaler / Netskope / 사내 MITM)이고, 바로 아래 섹션이 그 경우를 다룹니다.

PyPI 자체가 아예 막혀 있다면 uvx도 pip도 패키지에 닿을 수 없습니다. 사내 IT에 `pypi.org`와 `files.pythonhosted.org` 허용을 요청하거나, `pip install --index-url`로 가리킬 수 있는 사내 인덱스에 패키지를 미러링해 달라고 요청하세요.
...
| 소스 푸시 드리프트 (라이브 앵커 + 업데이트 셋 HOLD) | 편집한 소스를 `update_remote_from_local`로 되돌려 푸시할 때, 시간 윈도우로는 못 잡는 두 가지 검사를 추가합니다: 원격의 현재 `sys_updated_on`을 다운로드 시 기록한 값과 **시간 무관**하게 대조(수 시간, 심지어 **수 일** 뒤의 덮어쓰기도 포착), 그리고 레코드가 **다른 사용자의 미커밋 업데이트 셋에 잡혀(HOLD)** 있는지 실시간 확인. | 감지된 드리프트를 무시하고 푸시하려면 `force=true` |
...
| | 도구만 | 스킬 + 도구 |
|---|---|---|
| 안전성 | LLM 판단 (운빨) | 게이트 강제 (diff → 프리뷰 → confirm → 적용) |
| 토큰 | 소스 전문 컨텍스트 | 서브에이전트 위임, 요약만 반환 |
| 정확도 | 도구 순서 추측 | 검증된 파이프라인 |
| 롤백 | 까먹으면 끝 | 서버측 버전 히스토리(ServiceNow Versions 탭 / 업데이트 셋) |
...
| 카테고리 | 스킬 수 | 용도 |
|----------|---------|------|
| `analyze/` | 1 | **로컬 소스 검수** — 상호참조, 데드코드, 실행 순서, HTML 리포트 |
| `explore/` | 1 | **플로우 트리거 추적** — 테이블 변경 시 어떤 워크플로우/플로우가 실행되는지 |
| `manage/` | 2 | **앱 소스 다운로드**, **로컬 동기화** (diff → 충돌 탐지 후 푸시) |
...
safety_level: none|confirm|staged # → staged = diff/프리뷰/적용 필수
