...
Windows 的 [Smart App Control](https://support.microsoft.com/en-us/topic/what-is-smart-app-control-285ea03d-fa88-4495-afc7-c4d1abd9c0e0) 会让 uvx 完全无法运行：uvx 每次执行都会解包一个未签名的临时可执行文件，而 SAC 会拦截它。如果 uvx 在某次 Windows 更新之后突然不能用了，基本就是这个原因。此时改用 pip：
...
- **PyPI 可访问，但 HTTPS 被 TLS 检查**（Zscaler / Netskope / 企业 MITM）→ 见下方紧接着的 **在 TLS 检查代理后面安装**。
- **PyPI 本身被阻止** → 见更下方的 **Release zip/exe（本地安装）**。
...
当 PyPI 本身被企业安全阻止时，使用此路径。Release zip 提供一个 **PyInstaller 构建的单文件可执行程序**——无需 Python，无需安装脚本，无系统缓存污染。该可执行程序会自动检测位于其旁边的 `ms-playwright/` 目录，因此整个安装就是"解压并将你的 MCP 客户端指向它"。
...
复用[安装设置](https://github.com/jshsakura/mfa-servicenow-mcp#setup)中相同的客户端配置——只需将 `command` 改为你可执行程序的绝对路径、`args` 改为 `[]`，`env` 块保持不变。Claude Code 示例：
