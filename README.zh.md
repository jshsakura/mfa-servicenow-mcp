...
| 源码推送漂移（实时锚点 + update-set HOLD） | 用 `update_remote_from_local` 将编辑后的源码推回会增加两项时间窗口无法捕捉的检查：一项是将远程当前的 `sys_updated_on` 与下载时记录的值进行**与时间无关**的比较（捕捉数小时甚至**数天**之后的覆盖），以及一项对该记录是否被**保留在另一个用户未提交的 update set 中**的实时检查。 | 用 `force=true` 推送越过已检测到的漂移 |
...
| | 仅工具 | 工具 + 技能 |
|---|---|---|
| 安全 | LLM 决定 | 强制门控（diff → 预览 → confirm → 应用） |
| 令牌 | 源码转储在上下文中 | 委派给子代理，仅摘要 |
| 准确性 | LLM 猜测工具顺序 | 经过验证的流水线 |
| 回滚 | 可能忘记 | 服务器端版本历史（ServiceNow Versions 选项卡 / 更新集） |
...
安装程序会从本仓库的 `skills/` 目录下载技能文件，并将它们放在一个项目本地的 LLM 目录中。无需认证或配置。
...
| 分类 | 技能数 | 用途 |
|----------|--------|---------|
| `analyze/` | 1 | **本地源代码审计** —— 交叉引用、死代码、执行顺序、HTML 报告 |
| `explore/` | 1 | **flow 触发器追踪** —— 当某个表发生更改时哪些 workflow/flow 会触发 |
| `manage/` | 2 | **应用源码下载**、**本地同步**（diff → 推送，带冲突检测） |
