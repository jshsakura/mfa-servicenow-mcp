...
- [前置条件](https://github.com/jshsakura/mfa-servicenow-mcp#prerequisites)
...
这会安装 `uv`、获取并验证服务器，并下载 Chromium——一次性完成。获取时的 `--with playwright` 与下面的运行时配置相匹配，因此 uvx 会缓存完全相同的环境，使首次客户端启动瞬间完成。

> **引导式设置。** 不带任何标志运行 `servicenow-mcp setup` 会引导你完成编号菜单（按编号或名称选择客户端和认证类型——无需自由文本猜测），支持英语或韩语（根据你的区域设置自动检测；可用 `SERVICENOW_MCP_LANG=ko|en` 强制指定）。

### 2. 配置你的 MCP 客户端

将服务器添加到你客户端的配置文件中——从下方选择你的客户端。仅需两个环境变量；`MCP_TOOL_PACKAGE` 默认为 `standard`，因此除非你需要不同的工具包，否则可省略。
...
> 企业网络阻止 uvx/PyPI？请使用 [release zip/exe](https://github.com/jshsakura/mfa-servicenow-mcp#install-offline--corporate)。
...
对于大多数用户，上面的[安装设置](https://github.com/jshsakura/mfa-servicenow-mcp#setup)（uvx）即可满足所有需求。两种企业网络变体：

- **PyPI 可访问，但 HTTPS 被 TLS 检查**（Zscaler / Netskope / 企业 MITM）→ 见下方紧接着的 **pip install（TLS 检查后的内网）**。
- **PyPI / uvx 完全被阻止** → 见更下方的 **Release zip/exe（本地安装）**。

### pip install（TLS 检查后的内网——Zscaler 等）
...
### Release zip/exe（本地安装）

当 `uvx` 或 PyPI 被企业安全阻止时，使用此路径。Release zip 提供一个 **PyInstaller 构建的单文件可执行程序**——无需 Python，无需安装脚本，无系统缓存污染。该可执行程序会自动检测位于其旁边的 `ms-playwright/` 目录，因此整个安装就是"解压并将你的 MCP 客户端指向它"。

#### 1. 下载

可执行程序位于[最新发布](https://github.com/jshsakura/mfa-servicenow-mcp/releases/latest)。Chromium 包——仅在网络也阻止 Playwright 自身的 Chromium 下载时才需要——**不会**重新附加到每个发布版本中（它约 150 MB，且仅随 Playwright 变化）；请从长期存在的 [`chromium-bundle`](https://github.com/jshsakura/mfa-servicenow-mcp/releases/tag/chromium-bundle) 发布版本中获取。

| 平台 | 必需（最新发布） | 若 Chromium 下载被阻止，再添加此项（chromium-bundle 发布） |
|----------|---------------------------|------------------------------------------------------------------------|
...
选择任意你掌控的目录（`~/apps/servicenow-mcp/`、`D:\Tools\servicenow-mcp\` 等——只要保持稳定即可）。**预先解压两个 zip 文件**——不要把 `.zip` 文件留在可执行程序旁边。Chromium zip 解压出的目录只需以 `ms-play` 开头并包含一个 `chromium-*` 子目录；你的解压工具产生的任何名称都可以：
...
或者，如果你更想要一个干净的名称，可以解压到一个简单命名为 `ms-playwright/` 的文件夹。两者都可以——可执行程序在启动时会通配匹配任何同级的 `ms-play*` 目录，并在找到内部的 `chromium-*` 子目录后，**仅为当前进程**将 `PLAYWRIGHT_BROWSERS_PATH` 设置为该路径。它不会向磁盘任何位置写入，不会编辑你的 MCP 客户端配置，也不会触及系统级的 Playwright 缓存（`~/.cache/ms-playwright`、`%LOCALAPPDATA%\ms-playwright` 等）。如果未捆绑 Chromium，Playwright 会回退到自己的发现机制——你可以自己在 MCP env 中设置 `PLAYWRIGHT_BROWSERS_PATH`，或在某个可访问的位置运行 `playwright install chromium`。
...
如果版本号打印出来，二进制文件这一半就完成了——剩下的每一步都只是配置。

#### 4. 在你的 MCP 客户端中接入（复制粘贴）

复用与[安装设置](https://github.com/jshsakura/mfa-servicenow-mcp#setup)中 `uvx` 路径相同的客户端配置——只需将 `command` 改为你可执行程序的绝对路径，`env` 块保持不变。Claude Code 示例：
...
在 Windows 上，将 `"command"` 替换为 `"C:/Users/you/apps/servicenow-mcp/servicenow-mcp.exe"`。

> `SERVICENOW_USERNAME` / `SERVICENOW_PASSWORD` 是可选的（用于 MFA 登录预填充）。如果 Chromium 位于可执行程序旁边以外的某处，请在 env 块中添加 `"PLAYWRIGHT_BROWSERS_PATH": "/abs/path/to/ms-playwright"`。Codex（TOML）、OpenCode、Cursor、VS Code Copilot、Antigravity、Zed 的代码片段：[客户端设置指南](https://github.com/jshsakura/mfa-servicenow-mcp/blob/main/docs/CLIENT_SETUP.md)。

#### Chromium 回退（可选）

如果你跳过了 Chromium zip 而 Playwright 的自动下载被阻止，请在任何装有 Python 的机器上预先暂存该目录：
...
结果与捆绑 zip 产生的 `ms-playwright/chromium-*/…` 布局相同，因此自动检测会无需额外配置地拾取它。

> Windows 用户：关于 PATH 和杀毒软件的注意事项，请参阅 [Windows 安装指南](https://github.com/jshsakura/mfa-servicenow-mcp/blob/main/docs/WINDOWS_INSTALL.md)。
...
#### 在一个客户端中使用多个实例（dev / test / prod）

上面的示例是单实例的——这仍是默认设置。要在一个客户端中切换多个实例，请在 `SERVICENOW_INSTANCE_CONFIG`（别名 → 设置）中列出它们，并用 `SERVICENOW_ACTIVE_INSTANCE` 选择活动实例。每个别名都可以携带**自己的凭据**（`username` / `password` / `auth_type` / `api_key`）；`${ENV}` 引用可以把密钥排除在 JSON 之外。单实例的 `SERVICENOW_INSTANCE_URL` 形式仍可作为回退使用。
...
> **`uvx` 会缓存它下载的最后一个版本**并持续复用它。
> 要获取新发布版本，你必须显式刷新——它不会自行更新。
...
uvx 会解析最新的 `mfa-servicenow-mcp` 和 Playwright，而新的 Playwright 发布版本会附带新的 Chromium 构建。于是*首次*浏览器工具调用必须获取约 150 MB 的浏览器二进制文件——在慢速连接上这可能超过 MCP 主机的握手超时，并表现为：

```text
MCP startup failed: handshaking with MCP server failed: connection closed: initialize response
```

通过在首次调用**之前**安装 Chromium 来避免它（上面的设置命令已经做了这件事）：
...
uvx 会自动解析最新的 `mfa-servicenow-mcp` 和 `playwright`——你的配置中没有需要变更的版本号。要刷新：
...
> **我们为何不再在 MCP 服务器内部自动安装 Chromium：** 那个下载过去会在首次工具调用期间运行。在慢速连接上，子进程的存活时间超过了主机的握手截止时间，客户端便报告"connection closed"。v1.13.1 改变了这一点——MCP 服务器现在仅在缺少 Chromium 时*警告*。请提前用 `uvx --with playwright playwright install chromium`（带外执行，无握手计时器）安装它。
