...
Registered tools in the live registry: **66**
Packaged tool count in `full`: **55**
...
| `portal_developer` | 41 | standard plus portal, changeset, script include, and local-sync delivery workflows. |
| `platform_developer` | 41 | standard plus workflow, Flow Designer, UI policy, incident/change, and script writes. |
| `full` | 55 | Broadest packaged surface: all manage_* workflows plus advanced operations. |
