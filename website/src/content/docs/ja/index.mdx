...
<div class="section reveal" id="story">
  <div class="section-inner">
    <span class="section-label">なぜ作ったか</span>
    <h2 class="section-title">AIにパスワードを渡したくなかった。</h2>
    <p class="section-desc">
      それが本当の問題でした。AIエージェントをServiceNow内で動かすには、どの方法を選んでもエージェントが
      読めるどこかにパスワードやAPIキーを置く必要がありました。そこで代わりに、エージェントは実際の
      ブラウザセッションをそのまま引き継ぎます — MFA、SSO、SAMLなど、組織がすでに要求している方式
      そのままで — そして認証情報は一切見えません。
    </p>
    <p class="section-desc">
      そこから、これは単なるAPIラッパーではなく、一つのハーネスになりました — AIがただ呼び出すだけの
      ものではなく、任せられるものへ。すべての書き込みは、熟練したエンジニアが手作業で行うのと同じ
      手順を踏みます — ローカルファースト、プッシュ前にライブサーバーとdiff、競合する変更は静かに
      上書きされる代わりに検知されるようアンカリング、そして証明可能であること — デプロイXMLは
      手作業で編集したものではなく実際にライブインスタンスから来たことを示す証明書を伴います。
      トークンコストも同じ扱いを受けました — すべてのスキーマは圧縮され、すべてのパッケージは
      エージェントが実際に使える範囲だけに絞られています。
    </p>
    <p class="section-desc" style="margin-bottom:24px;">
      私はこれを自分の実際のServiceNow業務で毎日使っています。設計上の選択は仕様書からではなく、
      実践で壊れた部分から生まれています。
    </p>
    <div class="feature-grid reveal-stagger">
      <div class="step-card" style="--i:1">
        <h3>🔐 ゼロトラスト認証</h3>
        <p>エージェントはあなたのブラウザセッションを借ります。パスワードやAPIキーが設定ファイルに入ることは一切ありません。</p>
      </div>
      ...
      <div class="step-card" style="--i:4">
        <h3>🛠️ 毎日実際に使用</h3>
        <p>実際のServiceNow業務のために作られ、その上で使われています — ここにある粗さは実際にぶつかって磨かれたものです。</p>
      </div>
    </div>
  </div>
</div>
