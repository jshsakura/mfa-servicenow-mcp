...
<div class="section reveal" id="story">
  <div class="section-inner">
    ...
    <p class="section-desc">
      The Table API alone wasn't enough for that diagnosis. Flow Designer's real structure — triggers,
      actions, why a flow actually fired or didn't — lives behind session-only endpoints that ordinary
      REST auth can't reach. So the tool had to grow into real browser authentication, not an API key
      sitting in a config file. What an admin actually needs is what a human admin already has — a
      real, MFA/SSO/SAML-gated session. Nothing in the MCP ecosystem did that for ServiceNow, so that
      became the harness itself: the agent inherits a live browser session instead of holding a key.
    </p>
    <p class="section-desc">
    ...
    <div class="feature-grid reveal-stagger">
      ...
      <div class="step-card" style="--i:2">
        ...
        <p>Flow Designer's real structure lives behind session-only endpoints the Table API can't reach. The agent inherits a live admin session — MFA, SSO, SAML — instead of holding an API key.</p>
      </div>
