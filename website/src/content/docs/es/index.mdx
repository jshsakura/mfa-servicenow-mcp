...
<div class="section reveal" id="story">
  <div class="section-inner">
    <span class="section-label">Por qué existe esto</span>
    <h2 class="section-title">No quería entregarle una contraseña a mi IA.</h2>
    <p class="section-desc">
      Ese era el problema real. Cualquier forma de dejar que un agente de IA trabajara dentro de
      ServiceNow implicaba poner una contraseña o una clave de API en algún lugar que el agente
      pudiera leer. Así que, en cambio, el agente hereda una sesión de navegador en vivo — MFA, SSO,
      SAML, lo que sea que tu organización ya exija — y nunca ve una credencial.
    </p>
    <p class="section-desc">
      A partir de ahí dejó de ser un simple envoltorio de API y se convirtió en un harness: algo en lo
      que se puede confiar para que la IA opere, no solo para que lo invoque. Cada escritura pasa por
      la misma disciplina que usaría a mano un ingeniero cuidadoso — local-first, comparada (diff)
      contra el servidor en vivo antes de enviar nada, anclada para que una edición en conflicto se
      detecte en lugar de sobrescribirse en silencio, y demostrable, de modo que un XML de despliegue
      lleva un certificado que prueba que realmente vino de la instancia en vivo y no de una suposición
      editada a mano. El costo en tokens recibió el mismo trato — cada esquema comprimido, cada paquete
      acotado a lo que el agente realmente puede usar.
    </p>
    <p class="section-desc" style="margin-bottom:24px;">
      Uso esto todos los días en mi propio trabajo con ServiceNow. Las decisiones de diseño vienen de
      lo que se rompe en la práctica, no de una especificación.
    </p>
    <div class="feature-grid reveal-stagger">
      <div class="step-card" style="--i:1">
        <h3>🔐 Autenticación de confianza cero</h3>
        <p>El agente toma prestada tu sesión de navegador. Ninguna contraseña o clave de API llega jamás a un archivo de configuración.</p>
      </div>
      ...
      <div class="step-card" style="--i:4">
        <h3>🛠️ Usado a diario</h3>
        <p>Construido para trabajo real con ServiceNow, y usado en él — cada aspereza de aquí se pulió porque de verdad se topó con algo.</p>
      </div>
    </div>
  </div>
</div>
