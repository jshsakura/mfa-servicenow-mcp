...
<div class="section reveal" id="story">
  <div class="section-inner">
    ...
    <p class="section-desc">
      그런데 API 키는 이 용도에 맞지 않는 방식이라고 생각했습니다 — 설정 파일에 박혀서 원래
      쓰려던 작업이 끝난 뒤에도 계속 남아있는 정적인 비밀 값이니까요. 관리자에게 진짜 필요한 건
      실제 관리자가 이미 갖고 있는 것 — MFA/SSO/SAML로 보호되는 진짜 세션입니다. ServiceNow에
      대해 이걸 해주는 MCP가 없길래, 그게 이 프로젝트의 핵심 구조가 됐습니다 — 에이전트는 키를
      들고 있는 대신 살아있는 브라우저 세션을 그대로 물려받습니다.
    </p>
    <p class="section-desc">
    ...
    <div class="feature-grid reveal-stagger">
      ...
      <div class="step-card" style="--i:2">
        <h3>🔑 비밀 값이 아니라 세션</h3>
        <p>ServiceNow용 브라우저 인증을 지원하는 MCP가 없었습니다. 에이전트는 API 키 대신 살아있는 관리자 세션 — MFA, SSO, SAML —을 그대로 물려받습니다.</p>
      </div>
