...
- 가장 넓은 개발 패키지 `full`: **61**개 패키지 정의 도구
- 현재 어떤 패키지에도 묶이지 않은 등록 도구: **11**개
- 정확한 행 단위 전체 인벤토리: [영문 기준 문서](../../TOOL_INVENTORY/)
...
| `portal_developer` | 50 | standard + 포털/체인지셋/Script Include/로컬 동기화 개발 |
| `platform_developer` | 44 | standard + 워크플로우/플로우/스크립트/ITSM 쓰기 |
| `full` | 61 | 가장 넓은 패키지 표면 (번들 `manage_*` + 고급 운영 도구) |
