...
| 패키지명 | 도구 수 | ~토큰 | 사용 시기 |
| :--- | :---: | :---: | :--- |
| `core` | 12 | ~3.0K | 최소 읽기 전용: 헬스체크, 스키마, 탐색, 핵심 조회만 |
| `standard` | 31 | ~7.3K | **(기본값)** 인시던트/변경/포털/로그/소스 분석 읽기 전용 |
| `none` | 0 | 0 | 모든 도구를 의도적으로 비활성화 (테스트, 잠금 환경) |

⚠️ 쓰기 가능 — **고급 옵션**, 생성/수정/삭제 권한 부여:

| 패키지명 | 도구 수 | ~토큰 | 사용 시기 |
| :--- | :---: | :---: | :--- |
| `service_desk` | 33 | ~8.2K | ⚠️ 인시던트/변경 업데이트·종료가 필요한 서비스 데스크 담당자 |
| `portal_developer` | 50 | ~10.6K | ⚠️ 위젯, 체인지셋, Script Include를 배포하는 포털 개발자 |
| `platform_developer` | 44 | ~10.8K | ⚠️ 워크플로우, Flow Designer, 스크립트를 관리하는 플랫폼 엔지니어 |
| `full` | 61 | ~13.8K | ⚠️ 가장 고급 — 모든 도메인의 쓰기 도구 전체를 동시에 (아래 경고 참고) |

> **~토큰** = 요청마다 해당 패키지의 툴 스키마가 모델 컨텍스트에 더하는 대략적 토큰 수(tiktoken cl100k_base 기준, 실제 Claude 토큰 수는 약간 다름). 좁은 패키지를 쓸수록 컨텍스트와 비용이 절약됩니다.
