...
- **Descarga de adjuntos** (`download_attachment`) — guarda localmente los adjuntos y devuelve `saved_path`; las descargas pequeñas correctas también incluyen un `ResourceLink` MCP temporal para que clientes aislados obtengan el archivo sólo cuando lo necesiten, sin incluir base64 en el contexto LLM inicial
