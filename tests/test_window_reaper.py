...
def test_sidecars_are_not_mistaken_for_windows(mine, tmp_path):
    ...
    for sidecar in (".cursor", ".login", ".launches", ".impersonation"):
        ...
        for name in (
            f"debug_window_bob_at_ex_com{sidecar}.json",
            f"debug_window_bob_at_ex_com.test_example_com{sidecar}.json",
        ):
            (tmp_path / name).write_text("{}", encoding="utf-8")

    assert reaper.list_window_keys(mine) == ["bob_at_ex_com"]
...
def test_an_impersonating_window_survives(mine, tmp_path, monkeypatch, now):
    ...
    marker = tmp_path / "debug_window_bob_at_ex_com.impersonation.json"
    marker.write_text(
...
def test_an_idle_untouched_window_is_closed_and_reported(mine, tmp_path, monkeypatch, now):
    ...
    assert not os.path.exists(tmp_path / "debug_window_bob_at_ex_com.json")
...
def test_a_dead_window_leaves_no_state_behind(mine, tmp_path, monkeypatch, now):
    ...
    assert os.path.exists(tmp_path / "debug_window_bob_at_ex_com.json")
