...
class TestManageUxList(unittest.TestCase):
    ...
    def test_list_happy(self, mock_query):
        ...
        result = self._run(action="list", table="x_myapp_beta")
    ...
    def test_list_count_only(self, mock_count):
        ...
        result = self._run(action="list", table="x_myapp_beta", count_only=True)
        self.assertTrue(result["success"])
        self.assertEqual(4, result["count"])
        mock_count.assert_called_once_with(
            self.config, self.auth, "sys_ux_list", "table=x_myapp_beta"
        )
