...
def test_list_changesets_success(mock_config, mock_auth):
...
def test_list_changesets_with_filters(mock_config, mock_auth):
...
def test_list_changesets_exception(mock_config, mock_auth):
...
def test_get_changeset_success(mock_config, mock_auth):
    ...
    mock_auth.make_request.side_effect = [changeset_resp, changes_resp]
    resource = ChangesetResource(mock_config, mock_auth)
...
def test_get_changeset_exception(mock_config, mock_auth):
    mock_auth.make_request.side_effect = RequestException("not found")
    resource = ChangesetResource(mock_config, mock_auth)
    result = asyncio.run(resource.get_changeset("cs1"))
    parsed = json.loads(result)
    assert "error" in parsed
