...
## Unified Merge (3 MCP -> 1)

This root project is the unified result of the three legacy implementations.

- `servicenow-mcp-1` strengths merged: generic core tools (`sn_query`, `sn_aggregate`, `sn_schema`, `sn_discover`, `sn_health`) and natural-language helper (`sn_nl`)
- `servicenow-mcp-2` strengths merged: broad enterprise tool surface, package-based exposure (`MCP_TOOL_PACKAGE`), stdio/SSE transports
- `servicenow-mcp-3` strengths merged: pragmatic NLP parsing style and token/session lifecycle awareness

Security-first MFA stance:

- No MFA bypass logic is implemented
- Browser auth is interactive and MFA-compatible
- Browser session cookies are filtered to instance-scoped secure cookies only
...
### Browser Authentication (MFA/SSO-friendly)

Use this when tokens cannot be issued due to security policy and MFA must be completed manually.
The server opens a browser for login, waits for you to complete MFA/SSO, then captures the
...
Install Playwright (required for browser auth):
...
- `SERVICENOW_BROWSER_LOGIN_URL`: custom login URL (defaults to `{instance_url}/login.do`)
- `SERVICENOW_BROWSER_USER_DATA_DIR`: persist sessions between runs
