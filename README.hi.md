...
डिफ़ॉल्ट रास्ता **`uvx`** है — कोई अलग install चरण नहीं, यह बस चल पड़ता है। अधिकांश लोगों के लिए कहानी यहीं ख़त्म हो जाती है।
...
**अपडेट करने के लिए** — uvx अपने द्वारा डाउनलोड किए गए अंतिम version को cache करके उसी का पुनः उपयोग करता रहता है, इसलिए हर नए release को `--refresh` के साथ स्पष्ट रूप से खींचना पड़ता है:

```bash
uvx --refresh --with playwright --from mfa-servicenow-mcp servicenow-mcp --version
uvx --with playwright playwright install chromium     # नया Playwright आया हो तो नया Chromium build चाहिए
```
...
[python.org installer](https://www.python.org/downloads/) से लिया गया Python (signed, 3.10+) SAC से जस का तस गुज़र जाता है। इसे `servicenow-mcp` console script के बजाय `python -m servicenow_mcp` से चलाएँ — वह script pip द्वारा जनित एक unsigned `.exe` shim है, और SAC उसे भी ब्लॉक करता है।

> mac/Linux पर pip की एकमात्र चेतावनी यह है कि Homebrew और distro Pythons [PEP 668](https://peps.python.org/pep-0668/) के तहत global installs से इनकार कर देते हैं (`externally-managed-environment`)। python.org installer का उपयोग करें, या बस uvx पर बने रहें।

Chromium को **पहले से** install करना दोनों ही रास्तों में मायने रखता है। इसे पहले टूल कॉल तक टालने का मतलब है ~150 MB का डाउनलोड जो MCP host की handshake समय-सीमा से होड़ लगाता है — और यह `connection closed` के रूप में सामने आता है।

> **निर्देशित सेटअप।** बिना किसी फ़्लैग के `servicenow-mcp setup` (pip: `python -m servicenow_mcp setup`) चलाने पर यह आपको क्रमांकित मेनू के ज़रिए ले जाता है (क्लाइंट और auth प्रकार को नंबर या नाम से चुनें — कोई free-text अनुमान नहीं), अंग्रेज़ी या कोरियाई में (आपकी locale से स्वतः-पहचाना जाता है; `SERVICENOW_MCP_LANG=ko|en` से बाध्य करें)।

### 2. अपना MCP क्लाइंट कॉन्फ़िगर करें

सर्वर को अपने क्लाइंट की कॉन्फ़िग फ़ाइल में जोड़ें। **आपने चाहे जिस भी तरीके से install किया हो, `env` block एक जैसा ही रहता है** — केवल `command`/`args` उस रास्ते के हिसाब से बदलते हैं जो आपने ऊपर चुना:
...
यदि आपने pip से install किया है, तो केवल `command`/`args` इस तरह बदलें — बाकी सब वैसा ही रहता है:
...
### यदि आपका कॉर्पोरेट नेटवर्क install को ब्लॉक करता है

TLS-निरीक्षण करने वाले proxies (Zscaler और उसके जैसे) और ब्लॉक की गई PyPI पहुँच के लिए अलग रास्ता है — देखें [Install (ऑफ़लाइन / कॉर्पोरेट)](https://github.com/jshsakura/mfa-servicenow-mcp/blob/main/README.hi.md#install-ऑफ़लाइन--कॉर्पोरेट)।
...
- **अटैचमेंट डाउनलोड** (`download_attachment`) — अटैचमेंट को लोकल डिस्क पर सहेजकर `saved_path` लौटाता है; सफल छोटे डाउनलोड के साथ अल्पकालिक MCP `ResourceLink` भी मिलता है, ताकि अलग क्लाइंट शुरुआती LLM context में base64 डाले बिना ज़रूरत पर फ़ाइल को binary resource के रूप में प्राप्त कर सकें
...
अधिकांश उपयोगकर्ताओं के लिए ऊपर दिया गया [Setup](https://github.com/jshsakura/mfa-servicenow-mcp#setup) (uvx) ही पर्याप्त है। कॉर्पोरेट नेटवर्क के दो मामले ध्यान देने योग्य हैं।

सबसे आम स्थिति है **PyPI पहुँच योग्य है, लेकिन HTTPS TLS-निरीक्षित है** (Zscaler / Netskope / कॉर्पोरेट MITM) — ठीक नीचे वाला सेक्शन इसी को कवर करता है।

यदि PyPI स्वयं पूरी तरह ब्लॉक है, तो uvx और pip दोनों में से कोई भी पैकेज तक नहीं पहुँच सकता। अपनी IT टीम से `pypi.org` और `files.pythonhosted.org` को allowlist कराने के लिए कहें, या पैकेज को किसी आंतरिक index पर मिरर करवाएँ जिसकी ओर आप `pip install --index-url` से इंगित कर सकें।

### TLS-निरीक्षण करने वाले proxy के पीछे install (Zscaler आदि)
...
| | **A. प्रोफ़ाइल** (अनुशंसित) | **B. मल्टी-प्रोसेस** |
|---|---|---|
| सर्वर प्रक्रियाएँ | 1 | प्रति इंस्टेंस एक |
| क्लाइंट को दिखने वाले कनेक्शन | 1 | 3 |
| इंस्टेंस चुनना | प्रति कॉल `instance="test"` | प्रक्रिया पर fixed |
| इंस्टेंस-के-पार तुलना | **काम करती है** (`compare_instances`) | असंभव — प्रक्रियाएँ एक-दूसरे को जानती ही नहीं |
| ब्राउज़र लॉगिन | एक साझा सत्र | प्रति प्रक्रिया एक लॉगिन |
| Write नियंत्रण | प्रति alias `allow_writes` | प्रति-प्रक्रिया कॉन्फ़िग |

**अधिकांश लोगों को A चाहिए।** Write सुरक्षा वहाँ पहले से हल है — किसी prod alias पर `allow_writes` न लगाएँ और वह read-only हो जाता है, और किसी non-active इंस्टेंस पर write को `confirm_instance` गेट पार करना ही पड़ता है। इसके ऊपर से, इंस्टेंस-के-पार तुलना और एक ही लॉगिन केवल A में मिलते हैं।

**B तभी चुनें जब कनेक्शन क्लाइंट UI में दिखने में अलग-अलग होने चाहिए।** टूल के नाम `mcp_snow-prd_*` की तरह आते हैं, इसलिए इंसान एक नज़र में फ़र्क़ कर लेता है। क़ीमत है तीन लॉगिन, कोई तुलना नहीं, और कॉन्फ़िग की तीन प्रतियाँ। विवरण: [कई कनेक्शन में फ़र्क़ करना](https://github.com/jshsakura/mfa-servicenow-mcp/blob/main/README.hi.md#कई-कनेक्शन-में-फ़र्क़-करना---server-name)।

##### A. प्रोफ़ाइल

एक क्लाइंट से कई इंस्टेंस के बीच स्विच करने के लिए, उन्हें `SERVICENOW_INSTANCE_CONFIG` (alias → settings) में सूचीबद्ध करें और सक्रिय वाले को `SERVICENOW_ACTIVE_INSTANCE` से चुनें। प्रत्येक alias अपने **स्वयं के क्रेडेंशियल** (`username` / `password` / `auth_type` / `api_key`) रख सकता है; `${ENV}` संदर्भ secrets को JSON से बाहर रखते हैं। single-instance `SERVICENOW_INSTANCE_URL` रूप अभी भी fallback के रूप में काम करता है।
...
यह तभी सार्थक है जब आप कनेक्शन को क्लाइंट UI में अलग-अलग देखना चाहते हैं। हर entry **एक इंस्टेंस पर fixed** होती है और `--server-name` से उसे अपना नाम मिलता है:
...
इसके बाद टूल के नाम `mcp_snow-dev_*` / `mcp_snow-prd_*` पर fixed हो जाते हैं। `--server-name` हटा दें तो दोनों स्वयं को `ServiceNow` बताते हैं, इसलिए क्लाइंट उन्हें load क्रम से नंबर दे देता है (`mcp_servicenow`, `mcp_servicenow2`) — और वह क्रमांकन पुनः आरंभ के बीच बदल सकता है, यानी आप कभी भरोसा नहीं कर सकते कि कौन-सा production है।

production कनेक्शन को read-only रखने के लिए उसे एक read-only `MCP_TOOL_PACKAGE` दें। A के विपरीत यहाँ कोई `allow_writes` alias गेट नहीं है — **writes को रोकने वाली एकमात्र चीज़ tool पैकेज ही है।**

> लॉगिन प्रॉम्प्ट प्रति प्रक्रिया आते हैं, और `compare_instances` जैसे cross-instance टूल उपलब्ध नहीं होते — हर प्रक्रिया केवल अपना ही इंस्टेंस जानती है। यदि यह खटकता है, तो A चुनें।
...
| `--server-name` | `SERVICENOW_MCP_SERVER_NAME` | `ServiceNow` | क्लाइंट को बताया जाने वाला MCP सर्वर नाम |
...
#### कई कनेक्शन में फ़र्क़ करना (`--server-name`)

यदि आप एक ही क्लाइंट में **कई सर्वर entries** पंजीकृत करते हैं (dev / stg / prd अलग-अलग प्रक्रियाओं के रूप में), तो डिफ़ॉल्ट रूप से सबका नाम `ServiceNow` होता है, इसलिए क्लाइंट उन्हें load क्रम से अलग करता है — `mcp_servicenow`, `mcp_servicenow2`, `mcp_servicenow3`। यह क्रमांकन पुनः आरंभ के बीच बदल सकता है, जिससे **यह भरोसे लायक नहीं रहता कि कौन-सा production है।** इसके बजाय हर कनेक्शन को नाम दें:
...
इसके बाद tool namespace `mcp_snow-prd_*` पर fixed हो जाता है। `SERVICENOW_MCP_SERVER_NAME` env var से भी वही काम होता है, और दोनों सेट हों तो फ़्लैग जीतता है। सेट न हो तो नाम `ServiceNow` ही रहता है, इसलिए मौजूदा कॉन्फ़िग वैसे ही चलती रहती हैं।

> **एक ही** सर्वर के अंदर इंस्टेंस स्विच करना चाहते हैं? वह यह नहीं, [Multi-Instance Mode](#multi-instance-mode-comparison--guarded-single-call-writes) है। दोनों का आपस में कोई संबंध नहीं — `--server-name` वह नाम है जो क्लाइंट को दिखता है, जबकि multi-instance alias एक ही प्रक्रिया के भीतर किसी इंस्टेंस का नाम है।
...
आपने जिस तरीके से install किया, उसी से मेल खाता कमांड चुनें (यही कमांड [Install सेक्शन](#1-install) में भी हैं):

```bash
# uvx — यह अपने द्वारा डाउनलोड किए गए अंतिम version को cache करता है, इसलिए नया version --refresh से ही आता है
uvx --refresh --with playwright --from mfa-servicenow-mcp servicenow-mcp --version
...
दोनों ही स्थितियों में Chromium साथ-साथ refresh होता है, क्योंकि नया Playwright एक अलग Chromium build माँगता है (नीचे देखें)।

refresh करने के बाद, नए version को load करने के लिए **अपने MCP क्लाइंट को पुनः आरंभ करें** (Claude Code, Cursor, आदि)।

वर्तमान version जाँचें:
...
### Chromium को पहले से install क्यों करना पड़ता है

एक नया Playwright release एक अलग Chromium build माँगता है। यूँ ही छोड़ देने पर *पहले* ब्राउज़र टूल कॉल को ~150 MB ब्राउज़र बाइनरी fetch करनी पड़ती है — जो धीमे लिंक पर MCP host के handshake timeout को पार कर जाती है और इस रूप में सामने आती है:

```text
MCP startup failed: handshaking with MCP server failed: connection closed: initialize response
```

इसीलिए ऊपर दिए गए अपग्रेड कमांड हर बार `playwright install chromium` भी चलाते हैं।

> **हम अब MCP सर्वर के अंदर Chromium स्वतः-install क्यों नहीं करते:** वह डाउनलोड पहले टूल कॉल के दौरान चलता था। धीमे लिंक पर subprocess host की handshake समय-सीमा से अधिक जीवित रहा और क्लाइंट ने "connection closed" की रिपोर्ट दी। v1.13.1 ने इसे बदल दिया — MCP सर्वर अब केवल *चेतावनी* देता है यदि Chromium गायब है। इसे पहले से install करें (out-of-band, कोई handshake timer नहीं)।
...
|---|---|---|
| Safety | LLM निर्णय करता है | गेट लागू (diff → preview → confirm → apply) |
...
| Rollback | भूल सकता है | सर्वर-साइड version history (ServiceNow Versions टैब / update sets) |
...
installer इस repository की `skills/` डायरेक्टरी से skill फ़ाइलें डाउनलोड करता है और उन्हें एक project-local LLM डायरेक्टरी में रखता है। कोई प्रमाणीकरण या कॉन्फ़िगरेशन आवश्यक नहीं।

> यदि Windows पर सुरक्षा नीति `servicenow-mcp-skills` को ब्लॉक करती है, तो इसे module के रूप में कॉल करें — व्यवहार वही रहता है:
>
> ```bash
...
|----------|--------|---------|
| `analyze/` | 1 | **local source audit** — cross-references, dead code, execution order, HTML रिपोर्ट |
| `explore/` | 1 | **flow trigger tracing** — किसी table के बदलने पर कौन से workflows/flows चलते हैं |
| `manage/` | 2 | **app source download**, **local sync** (diff → conflict detection के साथ push) |
