...
## 2-A. 리뷰 4건이 전부 놓친 실제 결함 (v1.19.10-11 수정 완료)
...
### [FIX-3] 로그인 락 획득이 **원자적이지 않았다** (TOCTOU) — 치명, v1.19.11

리뷰들은 "파일 기반 동시성 제어가 어설프다"고 뭉뚱그렸을 뿐, 정작 **진짜 경합**은 짚지 못했다.

- `os.path.exists()`로 확인하고 `open(path, "w")`로 쓰는 사이가 비어 있었다.
  MCP 호스트 두 개가 동시에 뜨면 **둘 다 "락 없음"을 보고, 둘 다 쓰고, 둘 다 획득했다고 믿는다**
  → **브라우저 창 두 개**. 락이 막으라고 존재하는 바로 그 증상(이슈 #30).
- **수정**: `O_CREAT|O_EXCL` — 생성 자체가 곧 청구다. 커널이 승자를 하나만 고른다.
- **파생 함정 2개**(둘 다 테스트가 잡아냄, 설계 단계에선 안 보였다):
  1. stale 락을 N개가 **동시에** 읽으면 각자 지우고 각자 만든다 → 8개 중 5개가 이겼다.
     수거 구간을 별도 O_EXCL 마커로 **직렬화**해서 해결(마커도 나이로 자가 회수).
  2. O_EXCL 생성과 페이로드 쓰기는 **2개의 syscall**이라, 방금 이긴 락은 몇 µs간 **0바이트**다.
     이걸 "corrupt → 수거"로 읽으면 다시 두 명이 이긴다. **크기로 구분**:
     0바이트=진행 중인 청구(유예), 비어있지 않은데 파싱 실패=크래시 쓰레기(즉시 수거).

### [FIX-4] 형제의 토큰 회전이 **영구히 안 보이는** 창 (0.5초) — 높음, v1.19.11

`_maybe_adopt_sibling_session_update`에 `mtime <= watermark + 0.5` 관용 창이 있었는데,
**early return이 워터마크를 전진시키지 않는다.** 즉 형제가 우리 쓰기 직후 0.5초 안에 회전 토큰을
publish하면 → 스킵 → **그 다음 호출마다 같은 두 숫자를 영원히 재비교** → 영구 미채택.
우리는 죽은 `g_ck`를 계속 보내고 302 `/logout_success.do` → 재인증 루프.
**이 함수의 독스트링이 막겠다고 선언한 바로 그 루프다.** 관용 창은 업데이트를 **잃기만** 할 뿐
거짓 채택을 막아주지 않는다(reload가 어차피 내용 비교를 한다). 나노초 mtime + 무관용으로 교체.
부수: 워터마크를 **읽은 버전**으로 찍는다(reload 후 재stat하면 읽지 않은 버전을 찍어 또 잃는다).

### [FIX-5] 세션 캐시 쓰기 비원자성 + 스레드 충돌 — 중, v1.19.11

- `O_TRUNC` 제자리 쓰기였다. 형제들은 이 파일을 **mtime으로 폴링**하므로, 부풀린 mtime을 보고
  **반쯤 쓰인 크리덴셜**을 읽을 수 있었다 → tmp + `os.replace`로 원자화(권한 0600은 생성 시점에 부여).
- tmp 이름을 PID로만 지으면 **한 호스트의 두 스레드**(병렬 툴 호출 각각이 토큰 회전 흡수)가
  같은 tmp에 offset 0부터 겹쳐 쓰고 → `os.replace`가 그 쓰레기를 **원자적으로 게시**한다.
  → `threading.get_ident()` 추가.

### [FIX-6] invalidate가 형제의 **갓 만든 세션을 지울 수 있었다** — 중, v1.19.11

`invalidate_browser_session()`은 "디스크 쿠키가 내 것과 같을 때만 삭제"라는 올바른 의도를 갖지만
**읽기와 삭제 사이가 원자적이지 않다**. 게다가 `except: pass  # 못 읽으면 삭제해도 안전`이라
**일시적 OSError(EMFILE 등)에도 삭제**로 떨어졌다.
- 일시적 읽기 실패 → **삭제 금지**(로더들은 어차피 못 읽는 캐시를 "세션 없음"으로 처리해 재로그인).
- 내용이 진짜 쓰레기(JSON 파싱 실패) → 삭제 유지(기존 불변식, 테스트로 고정돼 있었다).
- 삭제 직전 mtime 재확인 → 우리가 들여다본 그 버전만 지운다.

> **교훈**: 이 6건 중 어느 것도 리뷰 4건이 잡지 못했다. 셋(FIX-3의 파생 함정 2개, FIX-5의 스레드
> 충돌)은 **수정하는 과정에서 새로 만든 버그**이고, 실제 스레드를 돌리는 테스트가 잡았다.
> 동시성 코드는 리뷰가 아니라 **경합을 실제로 재현하는 테스트**가 검증한다.
> → `tests/test_auth_lock_atomicity.py` (8스레드 배리어 경합 · 실파일시스템 · mock 없음).
