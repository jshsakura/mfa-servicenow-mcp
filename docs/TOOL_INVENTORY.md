...
Registered tools in the live registry: **107**
Packaged tool count in `full`: **75**
...
| `core` | 19 | Minimal read-only essentials for quick health/schema/table work. |
| `standard` | 51 | Default read-only package across incidents, changes, portal, logs, and source analysis. |
| `service_desk` | 52 | standard plus incident and change write workflows for operational support. |
| `portal_developer` | 62 | standard plus portal, changeset, script include, and local-sync delivery workflows. |
| `platform_developer` | 63 | standard plus workflow, Flow Designer, UI policy, incident/change, and script writes. |
| `full` | 75 | Broadest packaged surface: all manage_* workflows plus advanced operations. |
