...
Registered tools in the live registry: **68**
Packaged tool count in `full`: **57**
...
| `portal_developer` | 38 | standard plus portal, changeset, script include, and local-sync delivery workflows. |
| `platform_developer` | 43 | standard plus workflow, Flow Designer, UI policy, incident/change, and script writes. |
...
| `update_remote_from_local` | W | Push local edits to ServiceNow — run diff_local_component first to review. Path: sp_widget/<name>/<file> (folder) | s... | portal_developer, platform_developer, full |
