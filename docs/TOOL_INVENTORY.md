...
Registered tools in the live registry: **109**
Packaged tool count in `full`: **77**
...
| `core` | 21 | Minimal read-only essentials for quick health/schema/table work. |
| `standard` | 53 | Default read-only package across incidents, changes, portal, logs, and source analysis. |
| `service_desk` | 54 | standard plus incident and change write workflows for operational support. |
| `portal_developer` | 64 | standard plus portal, changeset, script include, and local-sync delivery workflows. |
| `platform_developer` | 65 | standard plus workflow, Flow Designer, UI policy, incident/change, and script writes. |
| `full` | 77 | Broadest packaged surface: all manage_* workflows plus advanced operations. |
