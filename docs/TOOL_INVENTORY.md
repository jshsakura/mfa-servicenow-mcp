...
Packaged tool count in `full`: **58**
...
| `portal_developer` | 47 | standard plus portal, changeset, script include, and local-sync delivery workflows. |
| `platform_developer` | 44 | standard plus workflow, Flow Designer, UI policy, incident/change, and script writes. |
| `full` | 58 | Broadest packaged surface: all manage_* workflows plus advanced operations. |
