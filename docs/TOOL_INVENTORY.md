...
Registered tools in the live registry: **108**
Packaged tool count in `full`: **76**
...
| `core` | 20 | Minimal read-only essentials for quick health/schema/table work. |
| `standard` | 52 | Default read-only package across incidents, changes, portal, logs, and source analysis. |
| `service_desk` | 53 | standard plus incident and change write workflows for operational support. |
| `portal_developer` | 63 | standard plus portal, changeset, script include, and local-sync delivery workflows. |
| `platform_developer` | 64 | standard plus workflow, Flow Designer, UI policy, incident/change, and script writes. |
| `full` | 76 | Broadest packaged surface: all manage_* workflows plus advanced operations. |
