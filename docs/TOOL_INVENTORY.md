...
Registered tools in the live registry: **70**
Packaged tool count in `full`: **56**
...
| `portal_developer` | 45 | standard plus portal, changeset, script include, and local-sync delivery workflows. |
...
| Tool | R/W | Description | Packages |
|------|-----|-------------|----------|
| `act_in_debug_window` | W | Drive the open debug window: click, fill, select, press, wait. Reports what the steps caused. | portal_developer |
| `inspect_debug_window` | R | Read the shared debug window: console errors, XHR, duplicate calls, screenshot, CSS. Never opens one. | portal_developer |
...
| Tool | R/W | Description | Packages |
|------|-----|-------------|----------|
| `query_local_graph` | R | Offline dependency/impact answers from audit graph files (0 API). uses|used_by|page|impact. | standard, portal_developer, platform_developer, service_desk, full |
...
| Tool | R/W | Description | Packages |
|------|-----|-------------|----------|
| `export_record_xml` | R | Export records as importable <unload> XML to disk (current sys_update_version). Read saved_path. | standard, portal_developer, platform_developer, service_desk, full |
