...
Registered tools in the live registry: **165**
Packaged tool count in `full`: **124**
Registered but currently unpackaged tools: **41**
...
| `service_desk` | 59 | standard plus incident and change write workflows for operational support. |
| `portal_developer` | 86 | standard plus portal, changeset, script include, and local-sync delivery workflows. |
| `platform_developer` | 99 | standard plus workflow, Flow Designer, UI policy, incident/change, and script writes. |
| `full` | 124 | Broadest packaged surface: all bundled manage_* workflows plus legacy compatibility wrappers. |
...
`add_group_members`, `create_article`, `create_category`, `create_epic`, `create_group`, `create_knowledge_base`, `create_project`, `create_scrum_task`, `create_story`, `create_story_dependency`, `create_user`, `delete_story_dependency`, `delete_workflow`, `get_article`, `get_developer_daily_summary`, `get_repo_file_last_modifier`, `get_repo_recent_commits`, `get_repo_working_tree_status`, `get_uncommitted_changes`, `get_user`, `get_workflow_activities`, `list_articles`, `list_categories`, `list_epics`, `list_groups`, `list_knowledge_bases`, `list_projects`, `list_scrum_tasks`, `list_stories`, `list_story_dependencies`, `list_users`, `list_workflow_versions`, `publish_article`, `remove_group_members`, `update_article`, `update_epic`, `update_group`, `update_project`, `update_scrum_task`, `update_story`, `update_user`
...
| `update_catalog_item` | W | Update a catalog item by sys_id. Partial field update. | full |

### Catalog Tools (7)

| Tool | R/W | Description | Packages |
|------|-----|-------------|----------|
| `create_catalog_category` | W | Create a catalog category. Requires title. Optionally set parent, icon, order, and active status. | full |
...
| `move_catalog_items` | W | Reassign one or more catalog items to a target category. Requires item_ids and target_category_id. | full |
| `update_catalog_category` | W | Partial update of a catalog category by sys_id. Supports title, parent, icon, order, and active fields. | full |

### Catalog Variables (3)

| Tool | R/W | Description | Packages |
|------|-----|-------------|----------|
| `create_catalog_item_variable` | W | Add a form variable to a catalog item. Requires cat_item sys_id, variable type, name, and label. | full |
...
| `update_catalog_item_variable` | W | Update a catalog variable by sys_id. Partial field update. | full |

### Change Tools (8)

| Tool | R/W | Description | Packages |
|------|-----|-------------|----------|
| `add_change_task` | W | Create a change_task under a change request. Requires change_id and short_description. | full |
| `approve_change` | W | Approve a change request and transition its state to implement. Requires change_id. | full |
| `create_change_request` | W | Create a change request. Requires short_description and type (normal/standard/emergency). | full |
...
| `update_change_request` | W | Update a change request by sys_id. Supports state, description, risk, impact, dates, and work notes. | platform_developer, full |
...
| Tool | R/W | Description | Packages |
|------|-----|-------------|----------|
| `add_comment` | W | Add a work note (internal) or customer-visible comment to an incident by sys_id or INC number. | service_desk, full |
| `create_incident` | W | Create a new incident (short_description required). Returns sys_id and INC number on success. | service_desk, full |
...
| `resolve_incident` | W | Set incident to Resolved state. Requires resolution_code and close_notes. Use update_incident for other state changes. | platform_developer, service_desk, full |
| `update_incident` | W | Update an incident by sys_id or INC number with partial field changes. Accepts any incident field. | platform_developer, service_desk, full |

### Knowledge Base (10)

| Tool | R/W | Description | Packages |
|------|-----|-------------|----------|
| `create_article` | W | Create a KB article. Requires kb_id, short_description, and text. | — |
...
| `publish_article` | W | Publish a KB article by sys_id. Sets workflow_state to published. | — |
| `update_article` | W | Update a KB article by sys_id. Supports title, text, category, and workflow_state fields. | — |
...
| Tool | R/W | Description | Packages |
|------|-----|-------------|----------|
| `create_angular_provider` | W | Create an AngularJS 1.x angular provider (factory/service/directive). Scope is required. | portal_developer, platform_developer, full |
| `create_column` | W | Add a column to a row. Columns use Bootstrap grid (size 1-12). Widgets are placed in columns. | portal_developer, platform_developer, full |
| `create_container` | W | Add a layout container to a portal page. Containers hold rows. | portal_developer, platform_developer, full |
| `create_css_theme` | W | Create a Service Portal CSS theme (sp_css). Scope is required. | portal_developer, platform_developer, full |
| `create_header_footer` | W | Create a Service Portal header or footer component. Scope is required. | portal_developer, platform_developer, full |
| `create_ng_template` | W | Create an AngularJS ng-template (sp_ng_template) for use in ng-include. Scope is required. | portal_developer, platform_developer, full |
| `create_page` | W | Create a new Service Portal page. Scope is required. Returns sys_id for subsequent layout creation. | portal_developer, platform_developer, full |
| `create_row` | W | Add a row to a layout container. Rows hold columns. | portal_developer, platform_developer, full |
| `create_ui_page` | W | Create a UI Page (sys_ui_page) with HTML, client script, and processing script. Scope is required. | portal_developer, platform_developer, full |
| `create_widget` | W | Create a new Service Portal widget with template, scripts, and CSS. Scope is required. | portal_developer, platform_developer, full |
...
| `update_page` | W | Update a Service Portal page's title, description, CSS, or visibility flags. | portal_developer, platform_developer, full |
...
| `update_portal_component` | W | Update specific code fields (HTML, CSS, or script) of a widget, provider, or script include. | portal_developer, full |
...
| Tool | R/W | Description | Packages |
|------|-----|-------------|----------|
| `create_script_include` | W | Create a script include with name, script, api_name, and client_callable fields. Returns sys_id. | portal_developer, platform_developer, full |
...
| `update_script_include` | W | Update a script include's script, api_name, client_callable, or other fields by sys_id or name. | portal_developer, platform_developer, full |
...
| Tool | R/W | Description | Packages |
|------|-----|-------------|----------|
| `create_ui_policy` | W | Create a form field behavior rule (show/hide/mandatory) triggered by encoded query conditions. | platform_developer, full |
| `create_ui_policy_action` | W | Add a field-level action to a UI policy: set visibility, mandatory, or read-only per field. | platform_developer, full |
...
| Tool | R/W | Description | Packages |
|------|-----|-------------|----------|
| `add_group_members` | W | Add one or more users to a group. Requires group sys_id and user sys_ids. | — |
| `create_group` | W | Create a group (sys_user_group). Requires name. Optional: manager, description, type. | — |
| `create_user` | W | Create a user (sys_user). Requires user_name. Optional: email, first/last name, roles. | — |
...
| `remove_group_members` | W | Remove one or more users from a group. Requires group sys_id and user sys_ids. | — |
| `update_group` | W | Update a group by sys_id. Supports name, manager, description, and active fields. | — |
| `update_user` | W | Update a user by sys_id. Supports name, email, active, department, and role fields. | — |
...
| `create_workflow` | W | Create a workflow with name, table, description, and active flag. Returns created record. | platform_developer, full |
