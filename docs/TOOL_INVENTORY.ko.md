...
- 전체 등록 도구: **64**
- 기본 패키지 `standard`: **27**개 읽기 전용 도구
- 가장 넓은 개발 패키지 `full`: **53**개 패키지 정의 도구
...
| `platform_developer` | 39 | standard + 워크플로우/플로우/스크립트/ITSM 쓰기 |
| `full` | 53 | 가장 넓은 패키지 표면 (번들 `manage_*` + 고급 운영 도구) |
