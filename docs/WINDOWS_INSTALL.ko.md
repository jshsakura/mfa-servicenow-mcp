...
| `standard` | 29 | **(기본값)** 인시던트/변경/포털/로그/소스 분석 전반의 읽기 전용 패키지 |
| `service_desk` | 31 | standard + 인시던트/변경 운영 쓰기 |
| `portal_developer` | 40 | standard + 포털, 체인지셋, Script Include, 로컬 동기화 워크플로우 |
| `platform_developer` | 45 | standard + 워크플로우, Flow Designer, UI Policy, 인시던트/변경/스크립트 쓰기 |
| `full` | 59 | 가장 넓은 패키지 표면: 번들 `manage_*` 워크플로우 + 고급 운영 도구 |
