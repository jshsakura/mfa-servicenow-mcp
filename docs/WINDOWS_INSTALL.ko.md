...
Windows에서도 다른 OS와 동일하게 `uvx`가 기본입니다. 다만 Windows에서만 생기는 두 가지 이유로 `uvx`를 못 쓰게 될 수 있습니다:

- **Smart App Control이 `uvx`를 차단** → **pip 설치**로 전환하세요 (1b단계). Windows에서 가장 흔한 고장 원인이고, 보통 Windows 업데이트 직후에 갑자기 나타납니다.
- **PyPI 자체에 접근 불가** (사내망 차단) → 최후의 수단으로 릴리즈 zip/exe (2단계).
...
바뀌는 건 `command`와 `args` 뿐입니다. **`env` 블록은 uvx 방식과 완전히 동일합니다** — 4단계의 설정을 그대로 복사한 뒤 위 두 줄만 바꾸면 됩니다:
...
Chromium 번들을 안 받았고 다운로드가 허용되는 환경이면 <https://www.python.org/downloads/> 에서 Python 3.10+을 설치한 뒤, 같은 구조로 캐시를 만드세요:
