...
| `standard` | 31 | ~7.3K | **（デフォルト）** インシデント、変更、ポータル、ログ、ソース分析にまたがる読み取り専用 |
...
- **デバッグブラウザーの常時接続**: 共有デバッグウィンドウは、ツール呼び出しごとにドライバーのサブプロセスと新しい WebSocket を作るのではなく、Playwright ドライバーとエンドポイント別 CDP 接続を保持する 1 本の長寿命ワーカースレッドで駆動されます。実際の Chromium での実測で、ウォーム呼び出しは約 2ms（従来方式は約 750ms）。キャッシュされた接続は再利用のたびにウィンドウ自身の DevTools エンドポイントで検証されるため、閉じられたウィンドウは古いハンドルで応答せず再接続します。
