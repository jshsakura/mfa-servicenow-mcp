...
これにより `uv` がインストールされ、サーバーが取得・検証され、Chromium がダウンロードされます — 一度だけです。取得時の `--with playwright` は下記のランタイム設定と一致するため、uvx は正確な環境をキャッシュし、最初のクライアント起動が瞬時に行われます。

> **ガイド付きセットアップ。** フラグなしで `servicenow-mcp setup` を実行すると、番号付きメニューでガイドされます（クライアントと認証タイプを番号または名前で選択 — 自由入力の推測は不要）。英語または韓国語に対応します（ロケールから自動検出。`SERVICENOW_MCP_LANG=ko|en` で強制可能）。

### 2. MCP クライアントの設定

クライアントの設定ファイルにサーバーを追加します — 下記からご自身のものを選んでください。必須の環境変数は 2 つだけです。`MCP_TOOL_PACKAGE` はデフォルトで `standard` になるため、別のパッケージが必要でない限り省略してください。
...
> 企業ネットワークが uvx/PyPI をブロックしていますか？ [release zip/exe](https://github.com/jshsakura/mfa-servicenow-mcp#install-offline--corporate) を使用してください。
...
- **65 個の登録済みツール** と **6 つのアクティブなパッケージプロファイル**、加えて無効化用の `none` — 最小限の読み取り専用から幅広いバンドル CRUD まで
- 安全ゲート、サブエージェント委譲、検証済みパイプラインを備えた **16 個のワークフロースキル**
...
ほとんどのユーザーにとって、上記の [Setup](https://github.com/jshsakura/mfa-servicenow-mcp#setup)（uvx）だけで十分です。企業ネットワーク向けに 2 つのバリエーションがあります:

- **PyPI には到達できるが、HTTPS が TLS インスペクションされている**（Zscaler / Netskope / 企業 MITM）→ すぐ下の **pip install（TLS インスペクション配下の社内ネットワーク）** を参照してください。
- **PyPI / uvx が完全にブロックされている** → さらに下の **Release zip/exe（ローカルインストール）** を参照してください。

### pip install（TLS インスペクション配下の社内ネットワーク — Zscaler など）
...
### Release zip/exe（ローカルインストール）

`uvx` または PyPI が企業のセキュリティでブロックされている場合に、このパスを使用します。release zip には **PyInstaller でビルドされた単一ファイルの実行可能ファイル** が同梱されます — Python 不要、インストーラースクリプト不要、システムキャッシュの汚染なし。この実行可能ファイルは自身の隣にある `ms-playwright/` ディレクトリを自動検出するため、インストール全体が「解凍して MCP クライアントを指し示すだけ」になります。

#### 1. ダウンロード

実行可能ファイルは [latest release](https://github.com/jshsakura/mfa-servicenow-mcp/releases/latest) にあります。Chromium バンドル — ネットワークが Playwright 自身の Chromium ダウンロードもブロックする場合にのみ必要 — は、毎リリースには再添付され **ません**（約 150 MB あり、Playwright とともにのみ変更されるため）。長期保管の [`chromium-bundle`](https://github.com/jshsakura/mfa-servicenow-mcp/releases/tag/chromium-bundle) リリースから取得してください。
...
自分が管理する任意のディレクトリ（`~/apps/servicenow-mcp/`、`D:\Tools\servicenow-mcp\` など — 安定していればどこでも構いません）を選びます。**両方の zip を最初に解凍してください** — `.zip` ファイルを実行可能ファイルの隣に放置しないでください。Chromium zip を解凍したディレクトリは `ms-play` で始まり `chromium-*` サブディレクトリを含んでいればよいだけで、解凍ツールが生成する名前は何でも構いません:
...
あるいは、すっきりした名前にしたい場合は、単に `ms-playwright/` というフォルダに解凍してください。どちらでも動作します — 実行可能ファイルは起動時に隣接する任意の `ms-play*` ディレクトリを glob で探し、その中に `chromium-*` サブディレクトリを見つけると、`PLAYWRIGHT_BROWSERS_PATH` をそのパスに **現在のプロセスに対してのみ** 設定します。ディスク上のどこにも書き込まず、MCP クライアントの設定も編集せず、システム全体の Playwright キャッシュ（`~/.cache/ms-playwright`、`%LOCALAPPDATA%\ms-playwright` など）にも触れません。Chromium がバンドルされていない場合、Playwright は独自の探索にフォールバックします — MCP の env で `PLAYWRIGHT_BROWSERS_PATH` を自分で設定するか、到達可能な場所で `playwright install chromium` を実行してください。
...
バージョンが表示されれば、バイナリ側は完了です — 残りのステップはすべて設定だけです。

#### 4. MCP クライアントに組み込む（コピー＆ペースト）

[Setup](https://github.com/jshsakura/mfa-servicenow-mcp#setup) の `uvx` パスと同じクライアント設定を再利用します — `command` を実行可能ファイルの絶対パスに変えるだけで、`env` ブロックは同一のままです。Claude Code の例:
...
Windows では `"command"` を `"C:/Users/you/apps/servicenow-mcp/servicenow-mcp.exe"` に置き換えてください。

> `SERVICENOW_USERNAME` / `SERVICENOW_PASSWORD` は任意です（MFA ログインの事前入力用）。Chromium が実行可能ファイルの隣以外の場所にある場合は、env ブロックに `"PLAYWRIGHT_BROWSERS_PATH": "/abs/path/to/ms-playwright"` を追加してください。Codex（TOML）、OpenCode、Cursor、VS Code Copilot、Antigravity、Zed のスニペット: [Client Setup Guide](https://github.com/jshsakura/mfa-servicenow-mcp/blob/main/docs/CLIENT_SETUP.md)。

#### Chromium フォールバック（任意）

Chromium zip をスキップし、Playwright の自動ダウンロードがブロックされている場合は、Python が使える任意のマシンでディレクトリを事前にステージングしてください:
...
結果はバンドル zip が生成するのと同じ `ms-playwright/chromium-*/…` 構成になるため、自動検出が追加設定なしで拾い上げます。

> Windows ユーザーへ: PATH とアンチウイルスに関する注意点は [Windows Installation Guide](https://github.com/jshsakura/mfa-servicenow-mcp/blob/main/docs/WINDOWS_INSTALL.md) を参照してください。
...
#### 1 つのクライアントで複数のインスタンス（dev / test / prod）

上記の例は単一インスタンスです — それがデフォルトのままです。1 つのクライアントから複数のインスタンスを切り替えるには、`SERVICENOW_INSTANCE_CONFIG`（エイリアス → 設定）にそれらを列挙し、`SERVICENOW_ACTIVE_INSTANCE` でアクティブなものを選びます。各エイリアスは **独自の認証情報**（`username` / `password` / `auth_type` / `api_key`）を持てます。`${ENV}` 参照によってシークレットを JSON の外に保てます。単一インスタンスの `SERVICENOW_INSTANCE_URL` 形式も引き続きフォールバックとして動作します。
...
| `core` | 12 | ヘルス、スキーマ、ディスカバリ、主要アーティファクトのルックアップ向けの最小限の読み取り専用エッセンシャル |
| `standard` | 27 | **（デフォルト）** インシデント、変更、ポータル、ログ、ソース分析にまたがる読み取り専用 |

⚠️ 書き込み可能（上級者向け — 作成/更新/削除を付与）:

| Package | Tools | Description |
| :--- | :---: | :--- |
| `service_desk` | 29 | ⚠️ standard + インシデントと変更の運用書き込み |
| `portal_developer` | 38 | ⚠️ standard + ポータル、changeset、script include、ローカル同期配信の書き込み |
...
> **`uvx` は最後にダウンロードしたバージョンをキャッシュ** し、それを再利用し続けます。
> 新しいリリースを取得するには明示的にリフレッシュする必要があります — 自動では更新されません。
...
### 最初のブラウザ呼び出しで Chromium がダウンロードされる

uvx は最新の `mfa-servicenow-mcp` と Playwright を解決し、新しい Playwright リリースには新しい Chromium ビルドが付属します。すると *最初の* ブラウザツール呼び出しで約 150 MB のブラウザバイナリを取得する必要があり、低速な回線では MCP ホストのハンドシェイクタイムアウトを超えてしまい、次のように表面化することがあります:

```text
MCP startup failed: handshaking with MCP server failed: connection closed: initialize response
```

最初の呼び出しの **前** に Chromium をインストールすることで回避できます（上記のセットアップコマンドは既にこれを行っています）:
...
uvx は最新の `mfa-servicenow-mcp` と `playwright` を自動解決します — 設定でバンプするバージョンはありません。リフレッシュするには:
...
> **なぜ MCP サーバー内で Chromium を自動インストールしなくなったのか:** そのダウンロードは以前、最初のツール呼び出し中に実行されていました。低速な回線ではサブプロセスがホストのハンドシェイク期限を超え、クライアントが「connection closed」と報告していました。v1.13.1 でこれが変更されました — MCP サーバーは Chromium がない場合に *警告するだけ* になりました。`uvx --with playwright playwright install chromium` で事前にインストールしてください（帯域外、ハンドシェイクタイマーなし）。
...
| ソースプッシュのドリフト（baseline + update-set HOLD） | 編集したソースを `update_remote_from_local` で書き戻す際、時間ウィンドウでは捕捉できない 2 つのチェックを追加します: リモートの現在の `sys_updated_on` を、ダウンロード時に記録された値と **時間に依存せず** 比較すること（数時間後、あるいは数 **日** 後の上書きを捕捉）、およびそのレコードが **別ユーザーの未コミット update set に保持されている** かのライブチェック。 | 検出されたドリフトを押し通すには `force=true` |
...
インストーラーはこのリポジトリの `skills/` ディレクトリから 24 個のスキルファイルをダウンロードし、プロジェクトローカルの LLM ディレクトリに配置します。認証や設定は不要です。
...
|----------|--------|---------|
| `analyze/` | 6 | Widget 分析、ポータル診断、provider 監査、依存関係マッピング、ESC 監査、**ローカルソース監査** |
| `fix/` | 3 | Widget パッチ適用（段階的ゲート）、デバッグ、コードレビュー |
| `manage/` | 8 | ページレイアウト、script include、ソースエクスポート、**アプリソースダウンロード**、changeset ワークフロー、ローカル同期、ワークフロー管理、**スキル管理** |
| `deploy/` | 2 | 変更要求のライフサイクル、インシデントトリアージ |
| `explore/` | 5 | ヘルスチェック、スキーマディスカバリ、ルートトレース、フロートリガートレース、ESC カタログフロー |
