...
Windows の [Smart App Control](https://support.microsoft.com/en-us/topic/what-is-smart-app-control-285ea03d-fa88-4495-afc7-c4d1abd9c0e0) が有効だと、uvx はそもそも起動できません。uvx は実行のたびに署名のない一時実行ファイルを展開するため、SAC がそれをブロックします。Windows Update の直後に突然 uvx が動かなくなった場合、原因はほぼこれです。そのときは pip を使ってください:
...
ほとんどのユーザーにとって、上記の [Setup](https://github.com/jshsakura/mfa-servicenow-mcp#setup) だけで十分です。企業ネットワーク向けに 2 つのバリエーションがあります:

- **PyPI には到達できるが、HTTPS が TLS インスペクションされている**（Zscaler / Netskope / 企業 MITM）→ すぐ下の **TLS インスペクションを行うプロキシ配下でのインストール** を参照してください。
- **PyPI 自体がブロックされている** → さらに下の **Release zip/exe（ローカルインストール）** を参照してください。
...
PyPI 自体が企業のセキュリティでブロックされている場合に、このパスを使用します。release zip には **PyInstaller でビルドされた単一ファイルの実行可能ファイル** が同梱されます — Python 不要、インストーラースクリプト不要、システムキャッシュの汚染なし。この実行可能ファイルは自身の隣にある `ms-playwright/` ディレクトリを自動検出するため、インストール全体が「解凍して MCP クライアントを指し示すだけ」になります。
...
[Setup](https://github.com/jshsakura/mfa-servicenow-mcp#setup) と同じクライアント設定を再利用します — `command` を実行可能ファイルの絶対パスに、`args` を `[]` に変えるだけで、`env` ブロックは同一のままです。Claude Code の例:
